...
// --- 2. Function to add a message to the chat body ---
function addMessage(text, sender) {
  const messageElement = document.createElement('div');
  messageElement.classList.add('chat-message', sender); // sender is 'user' or 'bot'
  ...
  return messageElement; // Return the element if we need to update it (e.g., for "loading")
}

// --- 3. Function to handle sending a message ---
async function handleSendMessage() {
  const sentence = chatInput.value.trim();
  if (sentence === '') return; // Don't send empty messages
  ...
  loadingMessage.classList.add('loading'); // Add loading style

  try {
    // Call our new "secure box" API
    const response = await fetch('/api/chat', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ sentence: sentence }), // Send the sentence as JSON
    });

    if (!response.ok) {
      throw new Error('AI server had an error.');
    }

    const data = await response.json();
    
    // Update the "..." message with the real answer
    loadingMessage.querySelector('p').textContent = data.corrected;
    loadingMessage.classList.remove('loading'); // Remove loading style

  } catch (error) {
    console.error('Error fetching from AI:', error);
    // Update the "..." message with an error
    loadingMessage.querySelector('p').textContent = 'Sorry, I had trouble connecting. Please try again.';
    loadingA.classList.remove('loading');
  }
}

// --- 4. Listen for Clicks and "Enter" key ---
if (chatSendBtn) {
